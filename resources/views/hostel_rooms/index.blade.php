@extends('layout.header') @section('content')
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel">Edit HostelRoom</h4>
    ...
    <div class="modal fade" id="warningModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <form class="modal-content" method="get" id="delete">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this HostelRoom?</p>
                </div>
    ...
    <div class="right_col" role="main">
        <div class="">
            ...
            <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                    <div class="x_title">
                        <h2><a href="/hostel_rooms/create" class="btn btn-sm btn-success"><i
                                    class="fa fa-plus text-white"></i> HostelRoom</a></h2>
                            ...
                            <div class="col-sm-12">
                                <div class="card-box table-responsive">
                                    <table id="datatable-buttons"
                                        ...
                                        <tbody>
                                            @foreach ($hostel_rooms as $hostelroom)
                                                <tr role="row" class="odd">
                                                    <td>{{ $hostelroom->id }}</td>
                                                    <td>{{ $hostelroom->name }}</td>
                                                    <td>{{ $hostelroom->room_number }}</td>
                                                    <td>{{ $hostelroom->number_of_students }}</td>
                                                    <td>{{ $hostelroom->status }}</td>
                                                    <td>{{ $hostelroom->hostel->name }}</td>
                                                    <td>{{ $hostelroom->hostelroomtype->name }}</td>
                                                    <td><a href="#" data-toggle="modal"
                                                            data-target=".bs-example-modal-lg"
                                                            onclick="initializeIframe('{{ route('hostel_rooms.edit', $hostelroom->id) }}')"><i
                                                                class="fa fa-pencil"></i> Edit </a> | <a
                                                            href="javascript:void(0)" data-toggle="modal"
                                                            data-target="#warningModal"
                                                            onclick="delete_id({{ $hostelroom->id }})"><i
    ...
    <script>
        function delete_id(id) {
            document.getElementById("delete").action = "/hostel_rooms/" + id + "/delete";
        }
