@extends('layout.header') @section('content')
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel">Edit Grading</h4>
    ...
    <div class="right_col" role="main">
        <div class="">
            ...
            <div class="col-md-12 col-sm-12 ">
                <div class="x_panel">
                    <div class="x_title">
                        <h2><a href="/gradings/create" class="btn btn-sm btn-success"><i class="fa fa-plus text-white"></i>
                                Grading</a></h2>
                                        ...
                                        <tbody>
                                            @foreach ($gradings as $grading)
                                                <tr role="row" class="odd">
                                                    <td>{{ $grading->id }}</td>
                                                    <td>{{ $grading->exam_type->name }}</td>
                                                    <td>{{ $grading->from_marks }}</td>
                                                    <td>{{ $grading->to_marks }}</td>
                                                    <td>{{ $grading->grade }}</td>
                                                    <td>{{ $grading->points }}</td>
                                                    <td><a href="#" data-toggle="modal"
                                                            data-target=".bs-example-modal-lg"
                                                            onclick="initializeIframe('{{ route('gradings.edit', $grading->id) }}')"><i
                                                                class="fa fa-pencil"></i> Edit </a> | <a
                                                            href="javascript:void(0)" data-toggle="modal"
                                                            data-target="#warningModal"
                                                            onclick="delete_id({{ $grading->id }})"><i
    ...
    <script>
        function delete_id(id) {
            document.getElementById("delete").action = "/gradings/" + id + "/delete";
        }
